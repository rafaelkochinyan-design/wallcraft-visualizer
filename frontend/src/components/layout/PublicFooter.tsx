...
export default function PublicFooter() {
  ...
  const whatsappNum = (tenant?.whatsapp || phone).replace(/\D/g, '')
  const instagram = tenant?.instagram_url || ''
  const facebook = tenant?.facebook_url || ''
  const tiktok = tenant?.tiktok_url || ''
  const pinterest = tenant?.pinterest_url || ''
