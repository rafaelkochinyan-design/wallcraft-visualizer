...
const WALL_PRESETS = [
  '#f8f8f6', '#f0ede4', '#e0ddd6',
  '#c0bdb4', '#3a3836', '#141412',
]
...
function PanelsTab() {
  const {
    ...
    setPanelInSlot, setHoverPanelId,
  } = useVisualizerStore()
  ...
  return (
    <div style={{ padding: '12px 10px' }}>
      ...
      <div style={{
        ...
        padding: '8px 10px', background: 'var(--ui-surface)', borderRadius: 12,
        marginBottom: 14, position: 'relative', minHeight: 56,
      }}>
        {selected ? (
          <>
            <img src={selected.thumb_url} alt={selected.name}
              style={{ width: 40, height: 40, borderRadius: 7, objectFit: 'cover', flexShrink: 0 }} />
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)', flex: 1 }}>{selected.name}</span>
            <button
          ...
          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Не выбрана — нажмите на панель ниже</span>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
        ...
        {availablePanels.length === 0 && (
          <p style={{ fontSize: 12, color: 'var(--text-muted)', gridColumn: '1/-1', padding: '12px 0', textAlign: 'center' }}>
            Нет панелей
          </p>
        )}
      </div>
...
function PanelCard({ panel, selected, onSelect, onHoverEnter, onHoverLeave }: {
  panel: Panel; selected: boolean
  onSelect: () => void; onHoverEnter: () => void; onHoverLeave: () => void
}) {
  ...
  return (
    <button
      ...
      style={{
        padding: 6, border: 'none', borderRadius: 12, cursor: 'pointer',
    ...
    >
      <div style={{
        ...
        overflow: 'hidden', background: 'var(--ui-surface)',
      }}>
        <img src={panel.thumb_url} alt={panel.name}
          style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
          onError={e => { (e.target as HTMLImageElement).style.display = 'none' }}
        />
      </div>
      <span style={{
        fontSize: 11, fontWeight: 500, color: selected ? 'var(--text-primary)' : 'var(--text-secondary)',
...
function AccessoriesTab() {
  const {
    ...
    placedAccessories, placeAccessory, removeAccessory,
  } = useVisualizerStore()
    ...
    <div style={{ padding: '12px 10px' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 14 }}>
        ...
        {availableAccessoryTypes.map(t => (
          <Chip key={t.id} label={t.label_ru} active={selType === t.id} onClick={() => setSelType(t.id)} />
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
        {filtered.map(acc => (
          <button key={acc.id}
            onClick={() => { placeAccessory(acc); toast(`Добавлен: ${acc.name}`) }}
            style={{
              padding: 6, border: '1px solid var(--ui-border)', borderRadius: 12, cursor: 'pointer',
              background: 'var(--ui-surface)', transition: 'background 0.12s',
              display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5,
              fontFamily: 'var(--font)',
            }}
          ...
          >
            <div style={{ width: '100%', aspectRatio: '1', borderRadius: 8, overflow: 'hidden', background: 'var(--ui-elevated)' }}>
              <img src={acc.thumb_url} alt={acc.name}
                style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                onError={e => { (e.target as HTMLImageElement).style.display = 'none' }}
              />
            </div>
            <span style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.2, textAlign: 'center' }}>
              {acc.name}
            </span>
          </button>
        ))}
        ...
        <div style={{ marginTop: 16 }}>
          ...
          {placedAccessories.map(p => (
            <div key={p.uid} style={{
              ...
              padding: '8px 10px', background: 'var(--ui-surface)', borderRadius: 9, marginBottom: 4,
            }}>
              <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{p.accessory.name}</span>
              <button onClick={() => removeAccessory(p.uid)} style={{
                background: 'none', border: 'none', cursor: 'pointer',
                color: 'var(--text-muted)', fontSize: 14, padding: '2px 4px', borderRadius: 5,
                transition: 'color 0.12s',
              }}
...
function SettingsTab() {
  ...
  return (
    <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <Slider label="Угол света"  value={lightAngle}     min={0}  max={360} unit="°" onChange={setLightAngle} />
      <Slider label="Высота"      value={lightElevation} min={5}  max={85}  unit="°" onChange={setLightElevation} />
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 7, marginBottom: 10 }}>
          {WALL_PRESETS.map(hex => (
            <button key={hex} onClick={() => setWallColor(hex)}
              style={{
                aspectRatio: '1', borderRadius: 9, border: 'none', cursor: 'pointer',
                background: hex,
                boxShadow: wallColor === hex
        ...
        <div style={{
          display: 'flex', alignItems: 'center', gap: 10,
          padding: '8px 10px', background: 'var(--ui-surface)', borderRadius: 10,
          border: '1px solid var(--ui-border)',
        }}>
          <div style={{
            width: 26, height: 26, borderRadius: 7, background: wallColor,
...
function Chip({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{
      padding: '5px 10px', border: 'none', borderRadius: 20, cursor: 'pointer',
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        <span style={{
          ...
          background: 'var(--ui-border)', padding: '1px 7px', borderRadius: 7,
