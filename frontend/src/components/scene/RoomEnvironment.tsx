...
function RoomLights({ rW, rH, rD }: { rW: number; rH: number; rD: number }) {
  return (
    <>
      ...
      <pointLight position={[0, rH - 0.1, rD * 0.35]} intensity={0.7} color="#fff4e0" distance={14} decay={2} />
      <pointLight position={[0, rH - 0.1, rD * 0.72]} intensity={0.55} color="#fff4e0" distance={12} decay={2} />
      {/* Заполняющий от пола */}
      <pointLight position={[0, 0.4, rD * 0.5]} intensity={0.07} color="#e8d8b8" distance={10} decay={2} />
    </>
  )
}
