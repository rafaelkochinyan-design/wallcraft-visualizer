...
export function TooltipWrapper({ children }: Props) {
  const {
    tooltipCollapsed, setTooltipCollapsed,
  ...
  const startY = tooltipPosition.y < 0
    ? (typeof window !== 'undefined' ? Math.round(window.innerHeight / 2 - 200) : 200)
    : tooltipPosition.y
  ...
  const panelSpring = useSpring({
    ...
    scaleY:   tooltipCollapsed ? 0.80 : 1,
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
        <button
          ...
          title="Свернуть"
        >
          {hov && <span style={{ fontSize: 9, color: '#6a0000', lineHeight: 1, fontWeight: 900 }}>—</span>}
        </button>
