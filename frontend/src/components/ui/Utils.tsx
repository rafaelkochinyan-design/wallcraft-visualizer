  ...
  return (
    <div style={{
      ...
      <div className="card-dark anim-fadein" style={{ padding: '14px 18px', minWidth: 190 }}>
        ...
        {totalPrice && (
          <Row label="~Стоимость" value={`${totalPrice} ₽`} accent />
        )}
      </div>
    </div>
  )
...
function Row({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
      <span style={{ fontSize: 'var(--text-xs)', color: 'rgba(255,255,255,0.38)', fontWeight: 600 }}>{label}</span>
      <span style={{ fontSize: 'var(--text-sm)', fontWeight: 700, color: accent ? '#4ade80' : '#fff' }}>{value}</span>
    </div>
  )
}
    ...
    <div style={{
      ...
      {error ? (
        <div style={{ textAlign: 'center' }}>
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 'var(--text-sm)', marginBottom: 16 }}>{error}</p>
          {onRetry && (
            <button onClick={onRetry} style={{
              padding: '10px 24px', background: 'rgba(255,255,255,0.1)',
              border: '1px solid rgba(255,255,255,0.12)', borderRadius: 'var(--r-full)',
              color: 'rgba(255,255,255,0.7)', cursor: 'pointer', fontSize: 'var(--text-sm)',
              fontFamily: 'var(--font)',
            }}>Повторить</button>
          )}
        </div>
