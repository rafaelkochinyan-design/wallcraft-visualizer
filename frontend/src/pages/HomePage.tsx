...
export default function HomePage() {
  ...
  const panelsByCategory = availablePanels.reduce<Record<string, { name: string; panels: Panel[] }>>(
    (acc, panel) => {
    ...
    <div>
      ...
      {!tenantLoading && availablePanels.length === 0 && (
        <section className="pub-section" style={{ textAlign: 'center', padding: '80px 32px' }}>
          ...
          <Link to="/products" className="pub-hero__cta" style={{ display: 'inline-flex', marginTop: 24 }}>
            {t('home.view_all')}
          </Link>
        </section>
      )}
      ...
      {/* ── Contact CTA ───────────────────────────────────── */}
      <section style={{ background: 'var(--ui-surface)', padding: '64px 32px', textAlign: 'center' }}>
