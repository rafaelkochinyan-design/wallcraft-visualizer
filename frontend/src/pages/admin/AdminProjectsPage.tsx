...
interface Project {
  ...
  cover_url: string | null; space_type: string | null
  active: boolean; sort_order: number; created_at: string
}

const SPACE_TYPES = ['living_room', 'bedroom', 'office', 'hotel', 'restaurant', 'bathroom', 'corridor']

function genSlug(title: string) {
  return title.toLowerCase().replace(/[^a-z0-9а-яё\s]/gi, '').replace(/\s+/g, '-').replace(/[а-яё]/gi, '').replace(/-+/g, '-').slice(0, 60)
}
...
export default function AdminProjectsPage() {
  ...
  async function load() {
    try { setItems((await api.get('/admin/projects')).data.data) } finally { setLoading(false) }
  }
  useEffect(() => { load() }, [])

  async function handleDelete(id: string) {
    if (!confirm('Удалить проект?')) return
    try { await api.delete(`/admin/projects/${id}`); showToast('Удалено'); load() }
    catch { showToast('Ошибка', 'err') }
  }

  return (
    <PageShell title="Проекты" addLabel="+ Новый проект" onAdd={() => { setEditing(null); setModalOpen(true) }} loading={loading}>
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Проект</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Slug</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Пространство</th>
          ...
          <tbody>
            ...
            {items.length === 0 && <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-400 text-sm">Нет проектов.</td></tr>}
          </tbody>
        </table>
      </div>
      <Toast toast={toast} />
      {modalOpen && <ProjectModal item={editing} onClose={() => setModalOpen(false)} onSaved={() => { setModalOpen(false); load(); showToast('Сохранено') }} onError={m => showToast(m, 'err')} />}
    </PageShell>
  )
}

function ProjectModal({ item, onClose, onSaved, onError }: { item: Project | null; onClose: () => void; onSaved: () => void; onError: (m: string) => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.title || !form.slug) { onError('Название и slug обязательны'); return }
    setSaving(true)
    try {
      item ? await api.put(`/admin/projects/${item.id}`, form) : await api.post('/admin/projects', form)
      onSaved()
  ...
  return (
    <Modal title={item ? 'Редактировать проект' : 'Новый проект'} onClose={onClose} wide>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <Field label="Slug *">
          <input className={`${inputClass} font-mono`} value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value }))} placeholder="my-project" />
        </Field>
        <Field label="Описание">
          <textarea className={`${textareaClass} h-24`} value={form.description || ''} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
        </Field>
        <Field label="Обложка">
