...
interface TeamMember {
  id: string; name: string; role: LocaleStr
  photo_url: string | null; bio: LocaleStr | null
...
export default function AdminTeamPage() {
  ...
  async function load() {
    try { setItems((await api.get('/admin/team')).data.data) } finally { setLoading(false) }
  }
  useEffect(() => { load() }, [])

  async function handleDelete(id: string) {
    if (!confirm('Удалить участника?')) return
    try { await api.delete(`/admin/team/${id}`); showToast('Удалено'); load() }
    catch { showToast('Ошибка', 'err') }
  }

  return (
    <PageShell title="Команда" addLabel="+ Добавить" onAdd={() => { setEditing(null); setModalOpen(true) }} loading={loading}>
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Сотрудник</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Должность (RU)</th>
          ...
          <tbody>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    {item.photo_url
                      ...
                      : <div className="w-9 h-9 rounded-full bg-gray-200 flex items-center justify-center text-gray-400 text-xs font-medium">{item.name[0]}</div>
                    }
            ...
            {items.length === 0 && <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-400 text-sm">Нет участников команды.</td></tr>}
          </tbody>
        </table>
      </div>
      <Toast toast={toast} />
...
function TeamModal({ item, onClose, onSaved, onError }: {
  item: TeamMember | null; onClose: () => void; onSaved: () => void; onError: (m: string) => void
}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.name || !form.role.ru) { onError('Имя и должность (RU) обязательны'); return }
    setSaving(true)
    try {
      item ? await api.put(`/admin/team/${item.id}`, form) : await api.post('/admin/team', form)
      onSaved()
          ...
          <div className="flex flex-col gap-3">
            <Field label={`Должность (${lang.toUpperCase()}) *`}>
              <input
                ...
                onChange={e => setForm(f => ({ ...f, role: { ...f.role, [lang]: e.target.value } }))}
