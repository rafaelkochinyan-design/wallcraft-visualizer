...
export default function AdminHeroSlidesPage() {
  ...
  async function load() {
    try {
      setItems((await api.get('/admin/hero-slides')).data.data)
    } finally {
      setLoading(false)
    }
  }
...
function SlideModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await (item
        ? api.put(`/admin/hero-slides/${item.id}`, form)
        : api.post('/admin/hero-slides', form))
      onSaved()
    } catch (err) {
      onError(apiErr(err))
    } finally {
          ...
          <div className="flex flex-col gap-3">
            ...
            <Field label={`Button text (${lang.toUpperCase()})`}>
              <input
                ...
                onChange={(e) => setLocale('cta_label', e.target.value)}
              />
            </Field>
          </div>
        </div>
