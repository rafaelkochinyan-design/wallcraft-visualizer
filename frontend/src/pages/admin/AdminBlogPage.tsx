...
export default function AdminBlogPage() {
  ...
  async function load() {
    try {
      setItems((await api.get('/admin/blog')).data.data)
    } finally {
      setLoading(false)
    }
  }
...
function BlogModal({
  ...
  function handleTitleChange(val: string) {
    setForm((f) => ({
      ...f,
      title: { ...f.title, [lang]: val },
      ...(!item && lang === 'ru' ? { slug: genSlug(val) } : {}),
    }))
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await (item ? api.put(`/admin/blog/${item.id}`, payload) : api.post('/admin/blog', payload))
      onSaved()
    } catch (err) {
      onError(apiErr(err))
    } finally {
          ...
          <div className="flex flex-col gap-3">
            ...
            <Field label={`Body (${lang.toUpperCase()})`}>
              <textarea
                ...
                onChange={(e) => setLocale('body', e.target.value)}
              />
            </Field>
          </div>
        </div>
